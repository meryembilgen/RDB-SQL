...
create 
view sales_info as -- istersek bunu view yapıp isterdiğimiz vakit kullanırız.
select c.customer_id,c.first_name,c.last_name,c.city,o.order_id,oi.list_price,p.product_name
from sale.customer c
inner join sale.orders o on c.customer_id=o.customer_id  -- create yapıyorsak full yapmak mantıklı
inner join sale.order_item oi on o.order_id = oi.order_id
inner join product.product p on oi.product_id=p.product_id


select * 
from sales_info  -- diye çağırırız.
