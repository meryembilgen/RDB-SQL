...
SELECT city, SUM(customer_id) AS customer_total_city
FROM sale.customer
GROUP BY city
...
SELECT order_id, SUM(list_price) AS list_price
FROM sale.order_item
GROUP BY order_id
...
SELECT TOP 1 order_id, AVG(list_price) AS avarage_price
FROM sale.order_item 
GROUP BY order_id
...
---Ýkinci sýraladýðýmýz sütun birincisine göre dana karýþýk, görmesi ve anlamasý daha zor. Biz brand_id leri karþýlaþtýrmadýðýmýz için ikinci sýralamaya ihtiyacýmýz yok. Birinci sýralamada list_price larý karsýlaþtýrýp anlamamýz daha kolay.  
